...
main = do 
    ...
    expr <- getLine
    putStrLn (getResult expr)
    wannaContinue 

wannaContinue :: IO () 
wannaContinue = do 
    putStrLn "Хотите ввести новое выражение ? (Y/N)"
    enterDecision
...
enterDecision = do
    ...
    helper decision
        where
            helper input
                |(map toUpper input) == "Y" = main
                |(map toUpper input) == "N" = return ()
                |otherwise = do
                    putStrLn "Для выбора введите Y или N"
                    enterDecision

getResult :: [Char] -> [Char]
getResult expr = solveToString . getSolve . getListOfTokens $ expr

solveToString :: Show a => Result a -> [Char]
solveToString (Solution a) = "Результат: " ++ show a
solveToString (Trouble a) = "Возникла проблема: " ++ a